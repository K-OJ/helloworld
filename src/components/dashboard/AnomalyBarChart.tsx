...
export function AnomalyBarChart({ items }: Props) {
  const { t } = useLang();
  const prevKey = t.prevLabel;
  const currKey = t.currLabel;
  ...
  const top5 = items
    ...
    .map((i) => ({
      ...
      [prevKey]: i.baseline_volume,
      [currKey]: i.target_volume,
  ...
  return (
    <div className="w-full">
      <p className="text-sm text-slate-500 mb-3">{t.chartTitle}</p>
      <ResponsiveContainer width="100%" height={260}>
        <BarChart data={top5} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
          ...
          <Bar dataKey={prevKey} fill="#cbd5e1" radius={[4, 4, 0, 0]} />
          <Bar dataKey={currKey} radius={[4, 4, 0, 0]}>
            {top5.map((entry, idx) => (
              <Cell
                key={idx}
