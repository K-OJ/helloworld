...
## 7. 상태 관리 및 데이터 페칭 고도화 (State Management & Fetching Strategy)

현재 대규모 제약 데이터(월간 배치) 렌더링 시 발생하는 Prop Drilling과 상태 파편화로 인한 성능 병목을 해결하기 위해, Zustand와 SWR을 결합한 4단계 렌더링 최적화 아키텍처를 적용했습니다.

**Step 1: 상태 성격에 따른 엄격한 분리 (State Audit)**
* **Server State (원본 데이터, AI 분석 결과):** SWR에 전면 위임하여 컴포넌트는 데이터 소유 없이 구독만 수행합니다.
* **Global Client State (진행률, 필터, 모달):** Zustand 스토어로 이관하여 중앙 통제합니다.
* **Local Client State (아코디언, 툴팁):** 해당 컴포넌트 내부의 `useState`로 캡슐화합니다.

**Step 2: Zustand 도메인 분리 및 Middleware 구축**
* 비즈니스 로직 스토어(`useQAStore`)와 UI 상태 스토어(`useUIStore`)로 도메인을 분리하여 관리 포인트를 최소화했습니다.
* 특히 UI 스토어에는 Zustand `persist` 미들웨어를 적용하여, 브라우저 새로고침 시에도 사용자의 필터/정렬 상태가 유지되도록 UX 연속성을 확보했습니다.

**Step 3: Prop Drilling 해소 및 Direct Subscription**
* 중간자(Middleman) 컴포넌트의 불필요한 Prop 전달을 제거하고, 최하위 컴포넌트(예: `ErrorCell`)가 Zustand 스토어를 직접 구독(Direct Subscription)하도록 리팩토링하여 결합도를 낮췄습니다.

**Step 4: Selective Rendering 및 Optimistic UI**
* Zustand의 선택적 구독(Selective Subscription)을 통해 1,000개의 행 중 상태가 변한 단일 행(Row)만 리렌더링되도록 최적화했습니다.
* AI 분석 완료 시 SWR의 `mutate()`를 통한 로컬 캐시 부분 갱신 및 Optimistic UI(낙관적 업데이트)를 적용하여 체감 반응 속도를 극대화했습니다.
