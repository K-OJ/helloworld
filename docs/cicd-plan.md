...
**[100% 자동화된 Zero-Downtime 롤백]** 프로덕션 배포 후 Smoke Test(Playwright E2E)가 실패할 경우, GitHub Actions의 `failure()` 조건부가 발동하여 Vercel CLI(`vercel rollback`)를 통해 이전의 안정된 Commit Hash 배포본으로 인간의 개입 없이 즉시 자동 롤백됩니다.
